...
createDirList(){
 ...
 if [ -f $ignoreDirs ]; then
  ...
  find ${searchDir} -name 'package.json'|grep -v node_modules | grep -v -f $ignoreDirs | sort > $npmFiles
 else
  find ${searchDir} -name 'package.json'|grep -v node_modules | sort > $npmFiles
 fi
